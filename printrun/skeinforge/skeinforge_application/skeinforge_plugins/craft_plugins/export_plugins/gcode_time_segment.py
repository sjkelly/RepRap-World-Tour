...
def getCharacterIntegerString( character, splitLine, step ):
...
class GcodeTimeSegmentRepository:
	...
	def __init__(self):
		...
		self.initialTime = settings.FloatSpin().getFromValue(0.0, 'Initial Time (s)', self, 5.0, 1.0)
		...
		self.timeStep = settings.FloatSpin().getFromValue(0.0, 'Time Step (mcs)', self, 1.0, 1000.0)
		self.xStep = settings.FloatSpin().getFromValue(0.0, 'X Step per mm', self, 1.0, 0.1)
		self.yStep = settings.FloatSpin().getFromValue(0.0, 'Y Step per mm', self, 1.0, 0.1)
		self.zStep = settings.FloatSpin().getFromValue(0.0, 'Z Step per mm', self, 1.0, 0.1)
		self.eStep = settings.FloatSpin().getFromValue(0.0, 'E Step per mm', self, 1.0, 0.1)
		settings.LabelSeparator().getFromRepository(self)
...
class GcodeTimeSegmentSkein:
	...
	def addCharacterInteger(self, character, lineStringIO, splitLine, step):
		"Add a character and integer to line string."
		characterIntegerString = getCharacterIntegerString(character, splitLine, step)
	...
	def parseLine(self, line):
		...
		self.addCharacterInteger('X', lineStringIO, splitLine, self.repository.xStep.value )
		self.addCharacterInteger('Y', lineStringIO, splitLine, self.repository.yStep.value )
		self.addCharacterInteger('Z', lineStringIO, splitLine, self.repository.zStep.value )
		self.addCharacterInteger('E', lineStringIO, splitLine, self.repository.eStep.value )
		#if zString == None:
		...
		#	extrusionDistance = self.flowRate 
		#self.addStringToLine(lineStringIO, 'E%s' % int(round(extrusionDistance * self.repository.eStep.value)))
